    ...
    if(action === 'add'){
      ...
      setChapters(chapters.map((chapter) =>{
        if(chapter.chapterId === chapterId){
          ...
          updatedContent.splice(lectureIdx, 1)
  ...
  const addLecture = () => {
    ...
    setChapters(chapters.map(ch => {
      if (ch.chapterId === currentChapterId) {
        const updatedLectures = [...ch.chapterContent, {
          ...newLecture,
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        <div className='flex flex-col gap-1'>
          ...
          value={courseTitle} placeholder='Type here'
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            ...
            value={coursePrice} placeholder='0' className='otline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' required/>
          </div>
          ...
          {chapters.map((chapter, chapterIdx) =>(
            <div key={chapterIdx} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                ...
                <span className='text-gray-500'>{chapter.chapterContent.length}Lecture</span>
                ...
                <div className='p-4'>
                  {chapter.chapterContent.map((lecture, lectureIdx)=> (
                    <div key={lectureIdx} className='flex justify-between items-center mb-2'>
                      <span>{lectureIdx + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins -
                        <a href={lecture.lectureUrl} target='_blank' className='text-blue-500'>Link</a>
                        - {lecture.isPreviewFree ? 'Free Preview' : 'Paid'}
                      </span>
                      <img src={assets.cross_icon} alt="" className='cursor-pointer'
                      onClick={()=> handleLecture('remove', chapter.chapterId, lectureIdx)}/>
                    </div>
                  ))}
